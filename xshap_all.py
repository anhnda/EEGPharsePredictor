...
from train_all_data import get_train_test_all
...
def xshap(model_id=1, test_id=1):
    ...
    for _, data in tqdm(enumerate(test_dataloader)):
        ...
        if params.OUT_3C:
            preds_3o.append(pred.reshape(pred.shape[0], -1, 3).detach().cpu())
        ...
        if ic % 100 == 0 and ic > 0:
            ...
            joblib.dump([xss, lbss, lbwss, shap_values, dataset.idx_2lb, epochess, predss, preds_3o],
    ...
    joblib.dump([xs, lbs, lbws, shap_values, dataset.idx_2lb, epochess, predss, preds_3o],
                "%s/xmodel_%s_%s.pkl" % (get_model_dirname(), MODEL_ID, TEST_ID))
